...
def run_experiment():
    data_dir = 'data/svhn'
    output_dir = "summary_pic/svhn/"
    n_local = [1, 1, 1, 1]
    delay_factor = [0, 0, 0.14]
    batch_size = 256
    epochs = 2
    shuffle = True
    ...
    feature_num = 960  # the number of feature needed to be selected
    ...
    client1_train_dataset = datasets.SVHN(data_dir, split='train', download=True, transform=transform)
    client1_test_dataset = datasets.SVHN(data_dir, split='test', download=True, transform=transform)
    client2_train_dataset = datasets.SVHN(data_dir, split='train', download=True, transform=transform)
    client2_test_dataset = datasets.SVHN(data_dir, split='test', download=True, transform=transform)
    client3_train_dataset = datasets.SVHN(data_dir, split='train', download=True, transform=transform)
    client3_test_dataset = datasets.SVHN(data_dir, split='test', download=True, transform=transform)
    ...
    server_party = ServeParty(model=server_model, loss_func=server_loss_func, optimizer=server_optimizer,
                              n_iter=n_local[0])
    client1_party = ClientParty(model=client1_model, optimizer=client1_optimizer, device=device, n_iter=n_local[1],
                                random=random)
    client2_party = ClientParty(model=client2_model, optimizer=client2_optimizer, device=device, n_iter=n_local[2],
                                random=random)
